...
#ifndef POLYSTAR_ARRAY_FUNCTIONS_HPP_
#define POLYSTAR_ARRAY_FUNCTIONS_HPP_
/*!
\file
\author Greg Tucker
...
operator X (const A<T>& a, const std::array<R,Nel>& b){\
  ...
  for (auto x: out.subItr(sh)) for (size_t i=0; i<Nel; ++i){\
    x.back() = static_cast<polystar::ind_t>(i);\
    out[x] Y b[i];\
  }\
  return out;\
}
...
//    debug_update("Incompatible lattices\n",a.lattice().string_repr(),"\n",b.lattice().string_repr());
  ...
  if (is_star){
    ...
    out *= polystar::math::two_pi; // to avoid making a copy of the array, compared to 2π*dot(...)
    return out;
  }
  assert( a.size(1) == 3 && b.size(1)==3 );
  assert( a.is_row_ordered() && b.is_row_ordered() && a.is_contiguous() && b.is_contiguous() );
...
//  verbose_update("Same-lattice dot product with metric ", met);
  if (1u==aN || 1u==bN){
    if (1u==aN) {
      for (polystar::ind_t i=0; i<bN; ++i) oarray.val(i,0) = same_lattice_dot(a, b.view(i), met);
    } else {
      for (polystar::ind_t i=0; i<aN; ++i) oarray.val(i,0) = same_lattice_dot(a.view(i), b, met);
    }
  } else {
    for (polystar::ind_t i=0; i<aN; ++i) oarray.val(i,0) = same_lattice_dot(a.view(i), b.view(i), met);
  }
  return oarray;
}
// #else
  ...
  */
  ...
  cat(const polystar::ind_t dim, const LV<T>& a, const BA<R>& b){
  ...
  template<class T, class R, template<class> class LV, template<class> class BA, class S = std::common_type_t<T,R>>
  std::enable_if_t<(isLatVec<T,LV>&&isBareArray<R,BA>), BA<S>>
  cat(const polystar::ind_t dim, const BA<R>& a, const LV<T>& b){
    return BA<S>(a.decouple()).append(dim, b.hkl());
  }
  // [bArray],[bArray] cat
  template<class T, class R, template<class> class L, class S = std::common_type_t<T,R>>
  std::enable_if_t<bareArrays<T,L,R,L>, L<S>>
  cat(const polystar::ind_t dim, const L<T>& a, const L<R>& b){
...
operator*(const std::array<R,9>& m, const L<T>& a){
  ...
  */
  ...
  abs(const L<T>& a){
    L<T> out(a.shape());
    for (auto x : a.subItr()) out[x] = polystar::utils::magnitude(a[x]);
    return out;
  }
  // abs(LatVec)
  template<class T, template<class> class L>
